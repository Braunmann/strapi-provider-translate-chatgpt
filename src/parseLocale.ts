...
function stripAndUpper(locale: string): { unstripped: string; stripped: string } {
...
function parseLocale(strapiLocale: string, localeMap = {}, direction = 'target'): string {
  ...
  switch (stripped) {
    ...
    default:
      ...
      else if (localeMap[unstripped]) possiblyUnstrippedResult = localeMap[unstripped];
      else if (localeMap[strapiLocale]) possiblyUnstrippedResult = localeMap[strapiLocale];
